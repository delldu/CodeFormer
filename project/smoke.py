...
def export_facegan_onnx_model():
    ...
    # 1. Run torch model
    model, device = image_face.get_facegan_model()
    ...
    input_names = [ "input" ]
    output_names = [ "output"]
    onnx_filename = "output/image_facegan.onnx"
    ...
    new_onnx_model, check = simplify(onnx_model)
    assert check, "Simplified ONNX model could not be validated"
    print("Simplified ...")
    model_info.print_simplifying_info(onnx_model, new_onnx_model)
    new_onnx_model = onnxoptimizer.optimize(new_onnx_model)
    print("Optimizered ...")
    model_info.print_simplifying_info(onnx_model, new_onnx_model)
    onnx.save(new_onnx_model, onnx_filename)
    graph = new_onnx_model.graph
...
def export_facedet_onnx_model():
    ...
    # 1. Run torch model
    model, device = image_face.get_facedet_model()
    B, C, H, W = 1, 3, 1024, 1024 # model.MAX_H, model.MAX_W
    model.to(device)

    #dummy_input = torch.randn(B, C, H, W).to(device)
    dummy_input = todos.data.load_tensor("images/0001.png").to(device)
    dummy_input = F.interpolate(dummy_input, size=[H, W], mode="nearest")
    ...
    onnx_filename = "output/image_facedet.onnx"
...
if __name__ == "__main__":
    ...
    if args.bench_mark:
        run_bench_mark()
    if args.export_onnx:
        export_facegan_onnx_model() # OK
        export_facedet_onnx_model() # OK
